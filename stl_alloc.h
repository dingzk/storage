...
namespace JJ {
    ...
    inline T* _allocate(ptrdiff_t size, T*) {
        ...
        if (size == 0) {
            ...
            real = round_up(size * sizeof(T));
        }

#ifdef DEBUG
    std::cout << "_allocate " << " size:" << size * sizeof(T) << " real: " << real << std::endl;
#endif
        std::set_new_handler(0);
//        T *tmp = (T *)(::operator new(real));
        T *tmp = (T *)(shm_alloc(real));
        if (tmp == 0) {
